...
> 🆕 **个人习惯/训练记录？** 试试 [`local/记录表.html`](local/)——单文件「记录本」：记录健身/学习/写作的**量**，自带可视化复盘（连续天数、热力图、30 天趋势、周环比斜率），自动保存本地文件、可选 Gitee/GitHub 私有仓库做手机↔电脑同步。用法见 [`local/README.md`](local/README.md)。
...
├── local/                     # 🆕 单文件本地记录表（自动保存本地 + 多设备同步，零部署）
